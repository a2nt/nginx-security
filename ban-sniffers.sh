...
cat /srv-service/conf.d/nginx/conf.d/block-ip.conf | head -n -1 > /tmp/block-ip.conf
cat /srv-service/logs/nginx/ban-sniffer.log | awk -F " - " 'NF {print $2"/32 1;"}' >> /tmp/block-ip.conf
